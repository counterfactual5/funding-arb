...
const zhCN: DocSection[] = [
  ...
  },
  {
    ...
    blocks: [
      {
        type: 'p',
        text: 'Settings → Strategy 写入 scripts/data/strategy_config.json。以下 CLI 启动时会自动合并该文件（阈值、scan_venues、fee 策略），无需手改模板 JSON：',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: '模板仍控制 parallelLegs、depthCheck、dry_run 等执行细节。EdgeX 账户验证：python3 scripts/tools/verify_edgex_live.py',
      },
    ],
  },
  {
...
const zhTW: DocSection[] = [
  ...
  },
  {
    ...
    blocks: [
      {
        type: 'p',
        text: 'Settings → Strategy 寫入 scripts/data/strategy_config.json。以下 CLI 啟動時會自動合併該檔案（閾值、scan_venues、fee 策略），無需手改模板 JSON：',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: '模板仍控制 parallelLegs、depthCheck、dry_run 等執行細節。EdgeX 賬戶驗證：python3 scripts/tools/verify_edgex_live.py',
      },
    ],
  },
  {
...
const en: DocSection[] = [
  ...
  {
    id: 'strategy-config',
    title: 'Strategy config (shared with dashboard)',
    blocks: [
      {
        type: 'p',
        text: 'Settings → Strategy writes scripts/data/strategy_config.json. These CLIs merge it on start (thresholds, scan_venues, fees):',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Templates still own parallelLegs, depthCheck, dry_run. EdgeX account check: python3 scripts/tools/verify_edgex_live.py',
      },
    ],
  },
  {
