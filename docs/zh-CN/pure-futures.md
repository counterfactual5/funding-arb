...
Pure Futures 是本系统的主策略：在两个交易所分别持有同一币种的永续多头与空头，赚取两所资金费率之差。不需要现货、不需要借币，天然跨所，Perp DEX（Hyperliquid / Aster / Lighter / EdgeX）也能参与。
...
min_edge_1h 更低是因为 1h 周期资金周转快、同周期无 timing risk；min_edge_mismatch 更高是为跨周期的结算不同步留风险溢价。
