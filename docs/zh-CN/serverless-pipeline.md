...
GitHub Actions 的 `workflow_dispatch` 不会自己按小时跑；需要外部 cron 服务调用 GitHub REST API。
...
**PAT 权限**：Repository access 选本仓库；Permissions → Actions → **Read and write**。
...
- `source: "cron"` 会传入 `--skip-if-unchanged`：Top-N 相对上次快照无 🆕/📈/📉 时不发 Telegram（demo 快照仍会更新）。
- 在 GitHub Actions UI 手动 Run workflow 时保持默认 `source=manual`，始终推送，便于调试。
- 任意步骤失败时 workflow 末尾的 `Alert on failure` 会向同一 `TELEGRAM_CHAT_ID` 发送带运行链接的告警。

> ⚠️ 若 cron-job.org 的 body 未包含 `"source": "cron"`，防刷屏逻辑不会生效，每小时都会无条件推送 Telegram。
