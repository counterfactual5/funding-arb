...
Scanner → Cash & Carry 表格支援 Dry-run 開倉：同所 spot + perp（forward 或 reverse，由該行費率符號決定）。API：POST /api/positions/open，strategy=carry，futures_venue 與 spot_venue 相同。

> ⚠️ 反向需交易所支援 margin 借賣（supports_reverse_arbitrage）。live 前確認借幣額度與利率；net_edge 已扣借幣成本但仍會浮動。
