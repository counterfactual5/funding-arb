...
def test_scan_spreads_net_apy():
    """Net APY = gross APY minus one-time cost (round-trip fees + entry basis)."""
    by_base = {
        "BTC": {
            "bitget": {
                "symbol": "BTCUSDT",
    ...
    assert (
        abs(row["net_apy_pct"] - (row["annual_apy_pct"] - row["round_trip_fee_pct"]))
        < 0.2
    )


def test_scan_spreads_net_apy_negative_when_no_funding():
    """Zero spread → no funding income → net_apy is negative (only fees)."""
    by_base = {
        "BTC": {
            "bitget": {
                "symbol": "BTCUSDT",
    ...
    if fwd:
        # gross APY = 0 (zero spread) → net_apy = -one_time_cost (fees) < 0
        assert fwd[0]["net_apy_pct"] < 0
