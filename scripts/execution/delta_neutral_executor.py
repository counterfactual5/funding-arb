...
def _margin_rollback_tags(spot_trade: dict[str, Any]) -> dict[str, Any]:
    """Reverse C&C 的现货腿走 margin：回滚单方向相反，借/还 side_effect 也要取反。"""
...
def execute_delta_neutral_trades(
    ...
    # 按照 pairs 分组，以保证每对 trades (现货 + 合约) 能组合处理
    ...
    for sym, sym_trades in pairs_trades.items():
        ...
        futures_trade = next((t for t in sym_trades if t["type"] in ("open_short", "close_short", "open_long", "close_long")), None)
        ...
        futures_rules = getattr(venue, "fetch_futures_symbol_rules", lambda x: None)(pair)
        if futures_rules:
            futures_trade["quantity_precision"] = futures_rules.get("quantity_precision", spot_trade.get("quantity_precision", 3))
            
            # Check min notional
            trade_usd = spot_trade["amount_base"] * market.get(sym, {}).get("price", 0.0)
            if trade_usd < futures_rules.get("min_trade_usdt", 0) or spot_trade["amount_base"] < futures_rules.get("min_trade_base", 0):
        ...
        if "open_" in futures_trade["type"]:
            ...
            if res_spot and res_spot[0].get("status") == "filled":
                ...
                exec_price = res_spot[0].get("exec_price", market.get(sym, {}).get("price", 0.0))
                ...
                transfer_amount = actual_trade_usd * 1.05 # Add some buffer for margin + fees
                if hasattr(venue, "transfer_asset"):
                    transfer_success = venue.transfer_asset("USDT", transfer_amount, "spot", "futures")
                ...
                if transfer_success:
                    ...
                    if not res_fut or res_fut[0].get("status") != "filled":
                        ...
                        rollback_res = venue.execute_trades([rollback_trade], market, dry_run=False)
            ...
            if res_fut and res_fut[0].get("status") == "filled":
                ...
                # Reverse 平仓：买回量对齐实际 margin 债务（borrowed+interest），
                # 用 AUTO_REPAY 一次清掉利息尘埃；偏离 >2% 视为异常数据不采用。
                if (
                    spot_trade["type"] == "buy"
                ...
                if not res_spot or res_spot[0].get("status") != "filled":
                    ...
                    rollback_type = "open_short" if futures_trade["type"] == "close_short" else "open_long"
                    ...
                    if hasattr(venue, "transfer_asset"):
                        spot_px = res_spot[0].get("exec_price", market.get(sym, {}).get("price", 0.0))
